...
-- customer. La paginación falla al pasar de 1,000 users (justo en el
-- evento del 11 de julio), causando Timeout 10s en Vercel y dejando sin
-- curso a clientes que pagaron.
--
-- Esta función consulta auth.users directamente (índice único en email)
...
--     (postgres), permitiendo leer auth.users. El owner es la única
--     identidad con acceso al schema auth.
--   - GRANT EXECUTE solo a service_role: el cliente (anon, authenticated)
--     nunca puede llamarla directamente — solo el server con service_role
