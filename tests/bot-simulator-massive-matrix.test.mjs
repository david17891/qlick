 ...
 *
 * Cubre:
 *   - `generateMassiveMatrix()` retorna ≥ 200 situaciones.
 *   - La distribución cartesiana es correcta (10 arquetipos × 4 contextos
 *     × 5 trayectorias = 200).
 *   - `auditMatrix()` corre las 200 situaciones en < 5 segundos.
...
test("M1: generateMassiveMatrix() retorna exactamente 200 situaciones", async () => {
  const { generateMassiveMatrix } = await import(MATRIX_URL);
  const matrix = generateMassiveMatrix();
  assert.equal(matrix.length, 200, "10 arquetipos × 4 contextos × 5 trayectorias");
});
...
test("M3: la distribución cartesiana es 10 arquetipos × 4 contextos × 5 trayectorias", async () => {
  ...
  assert.equal(contexts.size, CONTEXT_KEYS.length, "4 contextos");
...
test("M5: auditMatrix() corre 200 situaciones en < 5 segundos", async () => {
  ...
  assert.equal(report.passCount + report.failCount, 200);
...
test("M6: el reporte agrega correctamente por arquetipo", async () => {
  const { auditMatrix } = await import(AUDITOR_URL);
  const report = auditMatrix();
  // Cada arquetipo tiene 20 situaciones (4 contextos × 5 trayectorias).
  for (const [arch, agg] of Object.entries(report.byArchetype)) {
    assert.equal(
      agg.total,
      20,
      `arquetipo ${arch} debe tener 20 situaciones (4×5)`
    );
...
test("M8: el reporte incluye situationAudits con detalles por situación", async () => {
  const { auditMatrix } = await import(AUDITOR_URL);
  const report = auditMatrix();
  assert.equal(report.situationAudits.length, 200);
