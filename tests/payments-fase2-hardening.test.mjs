 ...
 *
 * Cubre los 4 vectores criticos:
 *  1. Regla de validacion de monto (anti-fraude): si session.amount_total
 ...
 *
 * Patron: tests unitarios de la logica pura (sin mocks profundos que son
 * fragiles). El flujo end-to-end se valida manualmente.
 *
...
/* =====================================================================
 * 1. WEBHOOK RECHAZA MONTO MENOR AL ESPERADO (regla de validacion)
 * =====================================================================
 *
 * La regla es:
...
test("regla monto webhook: amount_total > priceMXN * 100 => suspicious", () => {
  const priceMXN = 199;
  const receivedCentavos = 25000; // 250 MXN en lugar de 199 (cupon mal configurado o fraude)
  const expectedCentavos = Math.round(priceMXN * 100);
  const isSuspicious = receivedCentavos !== expectedCentavos;
  assert.equal(isSuspicious, true);
});
 ...
 * accion (windowMs: 60*60*1000, maxCalls: 3).
 */
...
test("rate limit: 3 calls/h OK, 4to rejected", () => {
  ...
  assert.ok(fourth.resetMs > 0, "resetMs debe indicar cuando se libera");
});
 ...
 * Stripe rechaza amount=0 con 400 Invalid amount, asi que la beca debe
...
test("applyCoupon: cupon inactivo => no discount", () => {
...
/* =====================================================================
 * 5. rate limit key format: contiene IP y email normalizado
 * =====================================================================
 */
