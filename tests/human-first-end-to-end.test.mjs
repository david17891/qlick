/**
 * E2E end-to-end del modo `human_first` (Sprint 2026-07-19, David
 * "human_first no estaba registrando en la base de datos").
 *
 * Que cubre:
 *   1. Flow "Nombre + email en el mismo mensaje":
 *      - confirmation creada (event_confirmations tiene fila).
 *      - leads.email actualizado.
 *      - leads.name actualizado.
 *      - email con QR enviado (mock de Brevo).
 *      - WhatsApp recibió mensaje con link de check-in.
 *
 *   2. Flow "email solo" (lead.name ya capturado en historial):
 *      - confirmation creada.
 *      - leads.email actualizado.
 *      - leads.name NO se sobreescribe (mantiene el del historial).
 ...
 *   - Mockea WhatsApp provider y Brevo sendEmail.
 *
 * TDD: este test es el primer paso. Los casos 1 y 2 deben FALLAR
 * (bugs #1 y #2 del safety-net) hasta que se implemente el fix.
 *
 * Uso:
 *   node --experimental-strip-types --experimental-test-module-mocks \
 *        --import ./tests/loader-register.mjs \
 *        --test tests/human-first-end-to-end.test.mjs
 */
...
// Asegurar que NO usamos DeepSeek real (queremos mock provider).
...
test("human_first E2E #2: flow 'email solo' -> intent = provide_email (case normal)", async () => {
  ...
  //   3. Para un evento de pago no se manda QR antes de verificar el pago.
  //
