...
function PreprintSheet({
  ...
  const certificateFolio = `CAN-2026-${String(printNumber).padStart(3, "0")}`;
  return (
    <div className="stage">
      <div className="cert">
        ...
        <div className="right">
          <div className="eyebrow-row"><div className="label">CONSTANCIA<div className="label-date">{formatDateLong(startsAt)}</div></div><div className="folio">FOLIO<div className="num">{certificateFolio}</div></div></div>
          <div className="hero"><div className="presented-to">Se otorga la presente a</div><div className={nameClass(name)}><span className="word">{plainName}</span><span className="word accent">{accentName}</span></div><div className="deco-line"><svg viewBox="0 0 20 20"><path d="M10 0l1.5 8.5L20 10l-8.5 1.5L10 20l-1.5-8.5L0 10l8.5-1.5z" fill="currentColor" /></svg></div><div className="reason">por su participación y registro en el programa de formación de Qlick Marketing Digital.</div></div>
          <div className="bottom"><div className={`sig-block${signatories.length > 1 ? " multiple" : ""}`}>{signatories.map((signatory) => <div className="signatory" key={signatory.name}><div className="signature"><img src={`/certificates/${signatory.assetFilename}`} alt={`Firma de ${signatory.name}`} /></div><div className="name" title={signatory.name}>{signatory.name}</div><div className="role">{signatory.title}</div></div>)}</div><div className="verify-block"><div className="label">Escanea</div><div className="qr"><img src={qrDataUrl} alt="QR de Qlick" /></div><div className="date">Constancia · {formatDateShort(new Date().toISOString())}</div></div></div>
        </div>
...
export default async function CertificatePreprintPage({ params, searchParams }: Props) {
  ...
  const [{ data: event }, { data: confirmations, error: confirmationsError }, { data: attendees, error: attendeesError }] = await Promise.all([
    ...
    supabase.from("event_attendees").select("id, name, confirmation_id, checked_in_at").eq("event_id", params.id).not("checked_in_at", "is", null),
  ]);
  if (!event || confirmationsError || attendeesError) notFound();

  const confirmationById = new Map(
    (confirmations ?? []).map((confirmation) => [confirmation.id, confirmation]),
  );
  const checkedInConfirmationIds = new Set(
    (attendees ?? []).map((attendee) => attendee.confirmation_id).filter(Boolean),
  );

  const attendeeRows: PreprintRow[] = (attendees ?? [])
    .filter((row) => {
    ...
    .map((row) => ({
      id: `attendee:${row.id}`,
      name: row.name!.trim(),
      paymentStatus: confirmationById.get(row.confirmation_id ?? "")?.payment_status ?? "attendee",
      kind: "attendee" as const,
    }));

  const preAttendeeRows: PreprintRow[] = (confirmations ?? [])
    .filter((row) => !checkedInConfirmationIds.has(row.id))
    .filter((row) => {
      const name = row.name.trim();
      return name.length >= 2 && !PLACEHOLDER_NAMES.has(name.toLowerCase());
    })
    .map((row) => ({
      ...
      kind: "pre_attendee" as const,
    }));

  const rows: PreprintRow[] = [...attendeeRows, ...preAttendeeRows]
    .sort((a, b) => a.name.localeCompare(b.name, "es"));

  const defaultSelectedIds = rows
    .filter((row) => row.kind === "attendee" || COVERED_PAYMENT_STATUSES.has(row.paymentStatus))
    .map((row) => row.id);
