...
> **Deploy 2026-08-18 — apartado unificado en CANACO:** `dpl_EMXsf3MwcTizeNdBrFzbg3RpZapD` quedó `READY` en producción con commit `9e2d394` y aliases `qlick.digital`/`www.qlick.digital`. `/promo` ofrece apartado de $200 MXN tanto para una persona ($1,000 total) como para dos ($1,500 total), y conserva el pago completo en ambas opciones. La migración aditiva `20260818090000_canaco_reservation_two_hundred.sql` está aplicada en Supabase Production: `reservation_amount_mxn=200`, saldo individual $800 y contexto CANACO actualizado. Smoke público: `/promo` y checkout individual con `payment_option=reservation|full` devuelven 200; los endpoints POST sin método/firma responden 405/403. Producción conserva `bot_global_mode=closing`, `bot_paused_global=false`, 2 confirmados y 19 pendientes; no se crearon cobros ni mensajes de prueba. Batería focalizada post-deploy: 272/272. CI conserva un único E2E externo omitido por falta de credenciales Supabase en el runner. Gmail: 59 mensajes de Qlick marcados `IMPORTANT` con etiqueta `Qlick/Alta prioridad`; la activación de notificaciones del sistema requiere Configuración de Gmail (el conector no expone filtros/notificaciones).
