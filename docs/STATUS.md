...
> **Deploy 2026-08-18 — saludo compacto de cierre:** `dpl_9KMsJDUh1ULApiCCtEUsKoxcyR4h` quedó `READY` en producción con commit `f0434c0`. El saludo muestra sólo contexto, promoción y botones; los datos de transferencia/OXXO aparecen únicamente después de pulsar ese botón. El botón de tarjeta entrega un solo `/promo`, sin Zoom ni beneficios no especificados. 18 pruebas focalizadas, type-check y lint pasan.

> **Deploy 2026-08-18 — pagos manuales visibles desde botones:** `dpl_5vrEhZLYSnMBMThaqYrAqA7YVcrU` quedó `READY` en producción con commit `0bf567e`. El modo global continúa en `closing`; el saludo no solicita nombre/correo ni captura desde WhatsApp. En vivo se verificó transferencia/OXXO, `LISTO` y “Ya pagué por OXXO” sin afirmar comprobante ni habilitar QR antes de revisión.
