...
>
> **Última actualización:** 2026-07-26 01:45 GMT-7 — El sprint de reglas del bot está implementado y verificado en la rama `codex/bot-rules-shadow-preview`, commit `478eb5d`. El flag de producción `bot_global_rules_enabled` sigue apagado y no se activó ninguna regla para leads reales. El preview usa `BOT_GLOBAL_RULES_SHADOW_ONLY=true` solo para el simulador y está desplegado en `https://qlick-9092iika4-david17891-9351s-projects.vercel.app` (Vercel `Ready`). Suite completa, type-check, lint y build pasan. Pendiente: probar el flujo visual autenticado del administrador y, después de aprobación explícita, ejecutar shadow/canary antes de activar producción. Ver `docs/ACTIVATION_GRADUAL_BOT_GLOBAL_RULES.md` para el rollout y `docs/PLAN_ROLLBACK_BOT_GLOBAL_RULES.md` para los escenarios de rollback.
>
...
- **Sprint "activar ai_bot_rules en el bot real" (26-jul):** commit `ecfa271` mantiene el loader de reglas aislado y el commit `478eb5d` corrige los E2E del bot, completa datos estructurados de eventos y resuelve el fallback de evento para que QR y confirmación queden enlazados. El loader fue probado contra Supabase real: shadow-only carga la regla sintética; el camino normal devuelve `[]`. La suite completa, type-check, lint y build están verdes. El flag de producción `system_settings.bot_global_rules_enabled` permanece `false` por diseño fail-closed. El preview usa `BOT_GLOBAL_RULES_SHADOW_ONLY=true` únicamente en la rama `codex/bot-rules-shadow-preview`; no hay activación para todos los leads. Ver `docs/ACTIVATION_GRADUAL_BOT_GLOBAL_RULES.md` y `docs/PLAN_ROLLBACK_BOT_GLOBAL_RULES.md`.
