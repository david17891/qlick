...
- `lead_info_followup_mode` es un switch independiente y actualmente está en `live` en producción.
- El rescate nuevo envía como máximo un mensaje después de 3 horas y respeta pausa, opt-out, respuestas manuales y la ventana de servicio de WhatsApp.
...
1. **Plantilla futura:** aprobar una plantilla antes de recuperar históricos que estén fuera de 24 horas y no tengan una señal de entrada de campaña de 72 horas. Por decisión actual, no se usan plantillas.
2. **Auditoría de entrega:** completar en la cola el `sent_at`, proveedor y resultado final de cada envío; el job ya persiste la campaña al enviar.
3. **Monitoreo operativo:** revisar la primera tanda automática y las respuestas desde la tarjeta de `Conversaciones`.
...
### Fase 1 — Recuperar automáticamente la ventana abierta
...
### Fase 2 — Recuperar históricos fuera de la ventana abierta
