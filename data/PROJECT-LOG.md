...
- **Pregunta:** David reportó (sesión 2026-07-06 ~14:55) "Bien hasta
  ahora, excepto por el mensaje extra, pero se llego todo el proceso"
  tras completar el flow completo del wizard (Q1→Q2→Q3→q_consent=Yes→
  q_business="Impresión 3d"). En su WhatsApp veía 2 mensajes de cierre,
  pero en la DB solo aparecía UN outbound (el thank-you).
- **Causa raíz (verificada via lead_whatsapp_conversations + código):**
  El fix F6 (audit 2026-07-06, justo antes de r4) agregó el send del
  follow-up bucket (HOT/MQL/coldWarm personalizado) al close path del
  wizard para simetría con /api/submit-survey. Pero:
  1. El close del wizard YA envía el thank-you estándar. Dos mensajes
     de cierre con copy similar = spam/confusión para el lead.
  2. El provider.send del bucket se hacía ANTES de retornar el plan del
     handler, con `await provider.send({ to, body })` directo — NO
     pasaba por el path normal de retorno (que sí persiste via
     `persistConversation`). Por eso aparecía en WhatsApp pero NO en
     la DB. Bug doble.
- **Decisión:** Remover el bloque follow-up bucket de survey_q4_text
  (líneas 2683-2723) y survey_q_consent_continue (líneas 2561-2583).
  ...
  desde el panel, o re-habilitar el código con la lógica revisada.
- **Asimetría con /api/submit-survey:** aceptada temporalmente. El
  ...
  Si en el futuro se quiere simetría, hay que refactorizar para que
  el bucket se envuelva en `persistConversation` y se persista.
- **Tests:** sin tests nuevos (cambio pequeño, lógica de bot bien
  cubierta por tests existentes). 535/535 verde.
- **Validación:** type-check ✓, lint ✓ (0 warnings), 535/535 tests ✓,
  build ✓.
